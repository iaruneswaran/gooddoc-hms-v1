      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Showing {visitDocuments.length} document{visitDocuments.length !== 1 ? 's' : ''} for visit {selectedVisit.visitId}
          </p>
        </div>
            ...
            <tbody className="bg-white dark:bg-card">
              {visitDocuments.map((doc) => (
                <tr key={doc.id} className="border-t hover:bg-muted/20 transition-colors">
                  <td className="p-3">
                    <p className="text-xs font-mono font-medium text-primary">{doc.documentNo}</p>
                  </td>
                  <td className="p-3">
