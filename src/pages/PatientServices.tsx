                ...
                <ScrollArea className="flex-1">
                  {totalPendingCount === 0 ? (
                    ...
                    <div className="divide-y divide-border">
                      {/* Bed Charges Section */}
                      {pendingBedCharges.length > 0 && (
                        <>
                          <div className="px-4 py-2 bg-cyan-500/5 border-b border-cyan-200">
                            <p className="text-xs font-semibold text-cyan-700 uppercase tracking-wider flex items-center gap-1.5">
                              <ArrowRightLeft className="w-3.5 h-3.5" />
                              Bed Transfer Charges
                            </p>
                          </div>
                          {pendingBedCharges.map((bc) => {
                            const isSelected = selectedPendingIds.has(bc.id);
                            const inCart = isInCart(bc.id);
                            
                            return (
                              <div 
                                key={bc.id}
                                onClick={() => togglePendingSelection(bc.id)}
                                className={cn(
                              ...
                              >
                                ...
                                <Checkbox
                                  checked={isSelected}
                                  onCheckedChange={() => togglePendingSelection(bc.id)}
                                  className="mt-1"
                                />
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    <span className="text-xs text-muted-foreground">{bc.fromRoomName}</span>
                                  </div>
                                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                                    <span className="flex items-center gap-1">
                                      <Clock className="w-3 h-3" />
                                      {format(new Date(bc.admissionDate), 'dd MMM')} → {format(new Date(bc.transferDate), 'dd MMM, HH:mm')}
                                    </span>
                                    <span>•</span>
                                    <span>₹{bc.fromTariff.toLocaleString('en-IN')}/day</span>
                                  </div>
                                  <p className="text-xs text-muted-foreground mt-1">
                                    Transferred to: {bc.toUnitName} • {bc.toBedName}
                                  </p>
                                </div>
                                ...
                                <div className="text-right flex-shrink-0">
                                  <p className="text-sm font-bold text-foreground">
                                    {formatPrice(bc.totalAmount)}
                                  </p>
                                  <p className="text-xs text-muted-foreground">
                                    {formatPrice(bc.fromTariff)} × {bc.daysStayed}
                                  </p>
                                  <p className="text-xs text-muted-foreground">+{bc.taxPct}% GST</p>
                                </div>
                              </div>
                            );
                          })}
                      ...
                      {/* Other Pending Services Section */}
                      {pendingServices.length > 0 && (
                        <>
                          {pendingBedCharges.length > 0 && (
                            <div className="px-4 py-2 bg-muted/50 border-b border-border">
                              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                                Other Pending Services
                              </p>
                            </div>
                          )}
